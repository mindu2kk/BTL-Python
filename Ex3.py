...
# Load the data (updated path assuming results.csv is in the same directory)
...
for col in percent_cols:
    if col in df.columns:
        df[col] = (
            df[col]
            .astype(str)  # Explicitly convert to string to ensure .str works
            .replace('N/a', np.nan)
...
non_stats_columns = ['Player', 'Nation', 'Team', 'Position', 'Age', 'Match played', 'Starts', 'Minutes']
...
with open('clustering_explanation.txt', 'w', encoding='utf-8') as f:
